...
impl GlyphKey {
    pub fn new(codepoint: u32, size: u16) -> Self {
        GlyphKey { codepoint, size }
    }
}
...
impl Font {
    ...
    pub fn get_name(&self) -> Cow<str> {
        unsafe { CStr::from_ptr((*self.ft_face).family_name) }.to_string_lossy()
    }
}
...
// created as textures
pub fn render_text_to_glyphs<'a, R, F>(
    text: Text,
    font: Font,
    ...
    F: Factory<R>,
{
    ...
    let r = render_text_to_glyphs_buf(text, font, buf, library, factory);
...
pub fn render_text_to_glyphs_buf<'a, R, F>(
    text: Text,
    font: Font,
    buf: *mut hb_buffer_t,
    library: &'a mut GlyphLibrary<R>,
