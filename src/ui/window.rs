pub fn main() {
    let builder = glutin::WindowBuilder::new()
        ...
        .with_dimensions(1024, 768) // Adjusted window dimensions
        .with_vsync();
    ...
    const TRI: [Vertex; 3] = [
        Vertex { pos: [-0.5, -0.5, 0.0, 1.0], color: [0.8, 0.2, 0.2] }, // Adjusted vertex colors
        Vertex { pos: [0.5, -0.5, 0.0, 1.0], color: [0.2, 0.8, 0.2] },
        Vertex { pos: [0.0, 0.5, 0.0, 1.0], color: [0.2, 0.2, 0.8] },
    ];

    let (vbuf, slice) = factory.create_vertex_buffer_with_slice(&TRI, ());
    let mut data = pipe::Data { vbuf, out: main_color };

    const CLEAR_COLOR: [f32; 4] = [0.1, 0.1, 0.1, 1.0]; // Adjusted clear color
